...
var ratingStuff:Array<Dynamic> = [
    ['You are stupid!', 0.2], //From 0% to 19%
    ['Told ya you're bad', 0.4], //From 20% to 39%
    ['No, you're bad', 0.5], //From 40% to 49%
    ['BRUH!', 0.6], //From 50% to 59%
    ['Eh.', 0.69], //From 60% to 68%
    ['Decent', 0.7], //69%
    ['This is fine.', 0.8], //From 70% to 79%
    ['Oh wow, can you believe it?', 0.9], //From 80% to 89%
    ['HOW PERFECT!', 1], //From 90% to 99%
...
function create() {
    ...
    timeTxt.setFormat(Paths.font("mvboli.ttf"), 32, FlxColor.WHITE, "center", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
    ...
    hudTxt = new FlxText(0, 685, FlxG.width, "Score: 0 | Breaks: 0 | Rating: ?");
    hudTxt.setFormat(Paths.font("mvboli.ttf"), 20, FlxColor.WHITE, "center", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
function update(elapsed:Float) {
    ...
    if (songScore > 0 || acc > 0 || misses > 0) {
        hudTxt.text = "Score: " + songScore + " | Breaks: " + misses +  " | Rating: " + rating + " (" + acc + "%)" + " - " + ratingFC;
    } 
}
